...
def ImportDSRLine(storecode,datetxt,dsrsale):
    ...
    for line in dsrsale:
        ...
        if accountno not in ["","N","$"]:
            ...
            customer_accounts = CustomerAccount.objects.filter(account_number=accountno)
            if len(customer_accounts) == 0:
                customer_account = CustomerAccount.objects.create( account_number=accountno, account_status="new", customer=customer, managed_by=store_org )
            else:
                customer_account = customer_accounts[0]
        elif loyaltyno != "":
            customers = Customer.objects.filter( customer_number=loyaltyno )
            if len(customers) == 0:
                ...
                customer_accounts = CustomerAccount.objects.filter(account_number=loyaltyno)
                if len(customer_accounts) == 0:
                    customer_account = CustomerAccount.objects.create( account_number=loyaltyno, account_status="new", customer=customer )
                else:
                    customer_account = customer_accounts[0]
        ...
        tills = PhysicalResource.objects.filter( owner=owner_role, serial_number=till_lineno, name="Till" )
        if len(tills) == 0:
            device = Device( owner=owner_role, serial_number=till_lineno, physical_object_type="Till", model=till_type )
            device.save()
            till = PhysicalResource.objects.create( owner=owner_role, serial_number=till_lineno, name="Till" )
            till.physical_objects.add(device)
        else:
            till = tills[0]
        ...
        if promo_code != "":
            ...
            promotions = Promotion.objects.filter( name=promo_code, valid_from=promo_start, valid_to=promo_end)
            if len(promotions) == 0:
                promotion = Promotion.objects.create( name=promo_code, valid_from=promo_start, valid_to=promo_end)
            else:
